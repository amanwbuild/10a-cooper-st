...
Case Studies is populated from portfolio project IDs in `site/data/template-config.js`. The current proposal features Mermaid, Canterbury and Terralsole in that order. Featured case studies are automatically removed from the later W. Portfolio page so projects are not repeated.
